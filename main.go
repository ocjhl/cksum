...
func crcer32(filePath string) string {
	...
	for i := uint64(0); i < blocks; i++ {
		...
		var j int
		for bsz := blockSize; bsz > 0; bsz = (bsz - 1) {
			var v uint32 = uint32(buf[j])
			crc = (crc << 8) ^ crctab[((crc>>24)^v)&0xFF]
			j++
		}
	}

	var length uint32 = uint32(fileSize)
	for ; length > 0; length >>= 8 {
		crc = (crc << 8) ^ crctab[((crc>>24)^length)&0xFF]
	}
	crc = ^crc & 0xFFFFFFFF
	return fmt.Sprintf("%d %d %s", crc, fileSize, filePath)
}
